...
const processQrCodesInBatch = async () => {
  ...
  for (const qrCode of currentQRCodes) {
    await renderAndDownloadQRCode(qrCode);
  }
...
// Render and download a QR code as SVG
const renderAndDownloadQRCode = (qrCode) => {
...
const generateZipFile = () => {
  ...
  useEffect(() => {
    if (isDownloadAllTriggered && renderedCount < qrCodes.length && currentPage <= totalPages) {
  ...
  return (
    <div className="container mx-auto px-4 py-6 relative">
      ...
      <div className="flex justify-center flex-wrap space-x-2 mt-6 overflow-x-auto whitespace-nowrap">
        <button
          onClick={() => handlePageClick(Math.max(pageWindowStart - visiblePageLimit, 1))}
        ...
        {Array.from({ length: Math.min(visiblePageLimit, totalPages - pageWindowStart + 1) }).map((_, idx) => (
          <button
        ...
        <button
          onClick={() => handlePageClick(Math.min(pageWindowStart + visiblePageLimit, totalPages))}
